                        ...
                        conn.query('INSERT INTO  project_member (project_id,user_id,project_member_role,project_member_status,project_application_reason)' +
                            ...
                            if (err) {
                                ...
                                if (rows.affectedRows == 1) {
                                    co(function *() {
                                        var project = yield db.query('SELECT * FROM project_info WHERE projectId = ?', [id]);
                                        console.log('project');
                                        var user = yield db.query('SELECT user_fullname FROM user WHERE user_id = ?', [userId]);
                                        console.log('user');
                                        var insertMessage = yield db.query('INSERT INTO mobile_message SET ?', {
                                            "sender_id": userId, //发送者的id
                                            "receiver_id": project[0][0].creatorId, //接受者id
                                            "status": 1,
                                            ...
                                            "content": user[0][0].user_fullname + '申请加入你的项目[' + project[0][0].projectName + ']。',
                                            "isPush": 2, //(1: 已经推送 2:未推送)
                                            "isCheck": 2, //(1: 已经查看 2:未查看),
                                            "createTime": (new Date()).toLocaleString(),
                                            "project_id": project[0][0].projectId, //消息相关项目的ID,
